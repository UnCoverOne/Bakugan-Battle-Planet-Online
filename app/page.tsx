      ...
      <section className="battle-board-column" aria-label="Battlefield">
        <div className="playmat-surface">
          ...
          <section className="opponent-table-row" aria-label="Opponent play area">
            <article className="shelf-zone deck-zone"><span>DECK</span><div className="card-pile card-back physical-stack"><b>{foe.deck}</b></div></article>
            ...
            <article className="shelf-zone opponent-team-zone"><span>BAKUGAN TEAM</span><div className="opponent-roster physical-character-row">{foe.bakugan.map((bakugan) => <article className={`${match.selected[foe.id] === bakugan.id ? "active" : ""} ${bakugan.open ? "open" : "closed"}`} key={bakugan.id} aria-label={`${bakugan.name}, ${match.selected[foe.id] === bakugan.id ? "selected" : bakugan.open ? "open" : "closed"}`}><div className="held-core-fan">{heldCoresFor(bakugan.id).map((placement, index) => <img src={placement.core.art} alt={placement.core.name} style={{ "--core-i": index } as React.CSSProperties} key={placement.cell} />)}</div><img className="physical-character-card" src={bakugan.art} alt={bakugan.name} /><span className="card-state-ribbon">{match.selected[foe.id] === bakugan.id ? "SELECTED" : bakugan.open ? "OPEN" : "CLOSED"}</span></article>)}</div></article>
            <article className="opponent-public-zones"><div className="opponent-energy"><span>ENERGY</span><strong>{foe.energy}/{foe.maxEnergy}</strong></div><div className="opponent-discard"><span>DISCARD</span><div className={`mini-discard-stack ${foeLatestDiscard ? "has-card" : "empty"}`}>{foeLatestDiscard ? <img src={foeLatestDiscard.art} alt={foeLatestDiscard.name} /> : <i /> }<b>{foe.discard.length}</b></div></div></article>
          </section>
          ...
          <section className="hex-arena" aria-label="BakuCore Field">
            ...
            <div className="battle-core-field axial">{match.placements.filter((placement) => !placement.attachedTo).map((placement) => { const cell = HEX_CELLS.find((candidate) => candidate.id === placement.cell)!; const targetable = match.phase === "target"; return <button key={placement.cell} title="Face-down BakuCore" aria-label="Face-down BakuCore" className={`${targetable ? "targetable" : ""} ${match.targets[playerId] === placement.cell ? "targeted" : ""} face-down`} style={{ "--q": cell.q, "--r": cell.r } as React.CSSProperties} disabled={!targetable} onClick={() => command("target", { cell: placement.cell }, (state) => targetCore(state, playerId, placement.cell))}><span className="core-back"><i /></span></button>; })}</div>
          </section>
          ...
          <section className="player-roster physical-character-row" aria-label="Your Bakugan">{me.bakugan.map((bakugan) => <button key={bakugan.id} className={`${match.selected[playerId] === bakugan.id ? "active" : ""} ${bakugan.open ? "open" : "closed"}`} disabled={match.phase !== "selection" || bakugan.open || !!match.selected[playerId]} onClick={() => command("select", { bakuganId: bakugan.id }, (state) => selectBakugan(state, playerId, bakugan.id))} aria-label={`${bakugan.name}, ${match.selected[playerId] === bakugan.id ? "selected" : bakugan.open ? "open" : "closed"}`}><div className="held-core-fan">{heldCoresFor(bakugan.id).map((placement, index) => <img src={placement.core.art} alt={placement.core.name} style={{ "--core-i": index } as React.CSSProperties} key={placement.cell} />)}</div><img className="physical-character-card" src={bakugan.art} alt={bakugan.name} /><span className="card-state-ribbon"><strong>{bakugan.name}</strong><em>{match.selected[playerId] === bakugan.id ? "SELECTED" : bakugan.open ? "OPEN" : "CLOSED"}</em></span></button>)}</section>

          <section className="player-zone-shelf" aria-label="Your physical card zones">
