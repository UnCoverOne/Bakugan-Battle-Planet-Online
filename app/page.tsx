...
import {
  ...
  type CardChoices, type GameCard, type MatchState,
} from "../lib/game";
...
function PlacementScreen({ match, playerId, faction, selectedCore, setSelectedCore, error, place, undo }: { match: MatchState | null; playerId: string; faction: string; selectedCore: string; setSelectedCore: (id: string) => void; error: string; place: (core: string, cell: string) => void; undo: () => void }) {
  ...
  return <section className="placement-page tabletop-placement" data-faction={faction.toLowerCase()}><header className="game-status"><div><Badge tone="gold">GAME {match.gameNumber}</Badge><strong>{match.format.toUpperCase()} • {Object.values(match.series).join("–")}</strong></div><div><span className="eyebrow">{match.stepLabel}</span><h1>{mine ? "PLACE A BAKUCORE" : "OPPONENT PLACING"}</h1></div><Timer deadline={match.deadline} /><button onClick={undo}>↶ UNDO REQUEST</button></header>
    <div className="placement-layout"><aside className="panel core-tray"><span className="eyebrow">YOUR UNUSED CORES</span><h2>SELECT ONE</h2>{player?.cores.map((core) => { const used = match.placements.some((p) => p.playerId === playerId && p.core.id === core.id); return <button disabled={used || !mine} className={selectedCore === core.id ? "selected" : ""} key={core.id} onClick={() => setSelectedCore(core.id)}><img src={core.art} alt={core.name} /><span>{core.name}</span>{used && <Badge>PLACED</Badge>}</button>; })}<p className="small-note">First Core is centre-only. Later Cores must touch the existing matrix.</p></aside>
      <main className="matrix-panel"><div className="matrix-heading"><div><span className="eyebrow">HIDE MATRIX</span><h2>{match.placements.length} / 12 CORES PLACED</h2></div><div className="orientation">↑ ROLL DIRECTION / FRONT</div></div><div className="hex-matrix">{HEX_CELLS.map((cell) => { const placed = match.placements.find((p) => p.cell === cell.id); const canPlace = mine && !!selectedCore && legal.includes(cell.id); return <button key={cell.id} className={`matrix-cell ${placed ? "occupied" : ""} ${canPlace ? "legal" : ""}`} style={{ "--q": cell.q, "--r": cell.r } as React.CSSProperties} disabled={!canPlace} onClick={() => { place(selectedCore, cell.id); setSelectedCore(""); }}>{placed ? <><img src={placed.core.art} alt={placed.core.name} /><span>{placed.order}</span></> : <i>{canPlace ? "+" : ""}</i>}</button>; })}</div><div className="matrix-legend"><span><i className="you" /> Your Core</span><span><i className="them" /> Opponent Core</span><span><i className="legal" /> Legal placement</span></div></main>
  ...
  return <section className="table-match battle-ui-v7" data-faction={faction.toLowerCase()}>
    ...
    <main className="tabletop battle-layout">
      ...
      <section className="battle-board-column" aria-label="Battlefield">
        ...
        <section className="opponent-table-row" aria-label="Opponent play area">
          <article className="shelf-zone deck-zone"><span>DECK</span><div className="card-pile card-back"><b>{foe.deck}</b></div></article>
          <article className="shelf-zone hand-zone"><span>HAND • {foe.hand.length}</span><div className="opponent-hand" aria-label={`${foe.hand.length} hidden cards`}>{foe.hand.slice(0, 8).map((card, index) => <i className="mini-card-back" style={{ "--i": index } as React.CSSProperties} key={`${card.id}-${index}`} />)}</div></article>
          <article className="shelf-zone opponent-team-zone"><span>BAKUGAN TEAM</span><div className="opponent-roster">{foe.bakugan.map((bakugan) => <article className={`${match.selected[foe.id] === bakugan.id ? "active" : ""} ${bakugan.open ? "open" : "closed"}`} key={bakugan.id}><img src={bakugan.art} alt={bakugan.name} /><div><strong>{bakugan.name}</strong><span>{bakugan.bPower}B • {bakugan.damage}D</span><em>{match.selected[foe.id] === bakugan.id ? "BRAWLING" : bakugan.open ? "OPEN" : "CLOSED"}</em></div></article>)}</div></article>
          <article className="opponent-public-zones"><div><span>ENERGY</span><strong>{foe.energy}/{foe.maxEnergy}</strong></div><div><span>DISCARD</span><strong>{foe.discard.length}</strong></div></article>
        </section>
        ...
        <section className="hex-arena" aria-label="BakuCore Field">
          ...
          <div className="battle-core-field axial">{match.placements.map((placement) => { const cell = HEX_CELLS.find((candidate) => candidate.id === placement.cell)!; const targetable = match.phase === "target" && !placement.attachedTo; return <button key={placement.cell} title={`${placement.core.name}${placement.attachedTo ? " • attached" : ""}`} className={`${targetable ? "targetable" : ""} ${match.targets[playerId] === placement.cell ? "targeted" : ""} ${placement.attachedTo ? "attached" : ""}`} style={{ "--q": cell.q, "--r": cell.r } as React.CSSProperties} disabled={!targetable} onClick={() => command("target", { cell: placement.cell }, (state) => targetCore(state, playerId, placement.cell))}><img src={placement.core.art} alt={placement.core.name} /><span className="core-label">{placement.attachedTo ? "HELD" : placement.core.type}</span></button>; })}</div>
          {foeBakugan && <figure className="active-brawler foe"><img src={foeBakugan.art} alt={foeBakugan.name} /><figcaption>{foeBakugan.name}<span>{foePower}B</span></figcaption></figure>}
          {myBakugan && <figure className="active-brawler me"><img src={myBakugan.art} alt={myBakugan.name} /><figcaption>{myBakugan.name}<span>{myPower}B</span></figcaption></figure>}
        </section>
