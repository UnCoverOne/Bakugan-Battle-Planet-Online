...
export function ensureRulesState(input: MatchState): RulesState {
  ...
  state.rules.triggerUsage = state.rules.triggerUsage && typeof state.rules.triggerUsage === "object"
    ? state.rules.triggerUsage
    : {};
  return state.rules;
}
...
export function normalizeRuleObjects(state: MatchState) {
  ...
  state.batch = state.batch.map((pending, index) => {
    ...
    return {
      ...
      independentChoiceSetId: `${pending.id}:choices:${index}`,
