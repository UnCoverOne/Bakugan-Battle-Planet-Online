...
export function evaluateBakuganCharacteristics(
  ...
  const coreModifiers: ContinuousModifier[] = effectiveBakucoreCells(state, bakugan, owner).flatMap((cell) => {
    ...
    if (bakugan.fused && core.fusionBonus) result.push({ id: `${core.id}:fusion-power`, source, controllerId: owner.id, target: "chosen-bakugan", targetBakuganId: bakugan.id, stat: "power", amount: core.fusionBonus, layer: "core", duration: "while-source-active", createdTurn: state.turn, sourceCategory: "bakucore" });
    if (bakugan.fused && core.fusionDamageBonus) result.push({ id: `${core.id}:fusion-damage`, source, controllerId: owner.id, target: "chosen-bakugan", targetBakuganId: bakugan.id, stat: "damage", amount: core.fusionDamageBonus, layer: "core", duration: "while-source-active", createdTurn: state.turn, sourceCategory: "bakucore" });
    ...
    if (bakugan.fused && core.fusionFrostStrike) result.push({ id: `${core.id}:fusion-frost`, source, controllerId: owner.id, target: "chosen-bakugan", targetBakuganId: bakugan.id, keyword: "FrostStrike", amount: core.fusionFrostStrike, layer: "core", duration: "while-source-active", createdTurn: state.turn, sourceCategory: "bakucore" });
