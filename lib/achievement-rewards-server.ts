...
export async function loadAchievementRewardAssignments(
  ...
  if (!row?.enabled) return normalizeAchievementRewardAssignments(DEFAULT_ACHIEVEMENT_REWARD_ASSIGNMENTS);
  return normalizeAchievementRewardAssignments(parseJson(row.data_json));
}
