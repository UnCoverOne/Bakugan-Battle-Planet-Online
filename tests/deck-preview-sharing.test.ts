...
test("public preview exposes the complete sharing and export surface", async () => {
  ...
  assert.doesNotMatch(exporter, /roundedRect\(context, OUTER - 24, 42, TEAM_RAIL_WIDTH \+ 48/);
  ...
  assert.match(exporter, /drawContainedImage\(context, image, x, y, size, size, false, false\);/);
  assert.match(exporter, /core\.set !== "Armored Alliance" \|\| core\.hasProvidedScan === true/);
  assert.match(exporter, /const CORE_OVERLAY_ICONS =/);
  assert.match(exporter, /bakuGear: "\/assets\/symbols\/baku-gear\.svg"/);
  assert.match(exporter, /drawCoreFallbackOverlay\(context, core, icons, x, y, size\);/);
  assert.match(exporter, /const FACTION_SYMBOLS: Record<string, string> =/);
  assert.match(exporter, /Aquos: "\/assets\/symbols\/factions\/aquos\.png"/);
  assert.match(exporter, /const factionIconSize = 34;/);
  assert.doesNotMatch(exporter, /deck\.factions\.join/);
  assert.doesNotMatch(exporter, /Main Deck cards`/);
  assert.match(exporter, /data-deck-creator-identity/);
  assert.match(exporter, /deck\.sourceCreator\?\.trim\(\) \|\| deck\.creator\?\.trim\(\)/);
