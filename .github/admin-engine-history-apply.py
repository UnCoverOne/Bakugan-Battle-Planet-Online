...
Path("lib/admin-engine-history-server.ts").write_text('''import type { AccountDatabase } from "./account-server";
...
replace_once(
    "components/game-screen-v2/GameMenuHud.tsx",
    '''  completed = false,
  undoAvailable,
''',
    '''  completed = false,
...
with Path("tests/admin-ai-visibility.test.ts").open("a") as file:
    file.write('''
