...
export function AppProvider({ children }) {
  ...
  const selection = useCallback((deck) => ({ playerId, name: profile.name, deck: { id: deck.id, name: deck.name, bakuganIds: [...deck.bakuganIds], coreIds: [...deck.coreIds], cardIds: [...deck.cardIds], format: deck.format, factions: [...(deck.factions ?? [])], leadCardId: deck.leadCardId } }), [playerId, profile.name]);
  ...
  const nextSeriesGame = useCallback(async () => { if (!match) return; try { let state; if (online) state = await api("next-game"); else { const { startNextSeriesGame } = await import("../../lib/game"); state = startNextSeriesGame(match); setMatch(state); } router.push(state?.ranked?.stage === "select" ? "/play/lobby" : "/play/match"); } catch (error) { setMatchError(error.message); } }, [api, match, online, router, setMatch]);
