...
function CardImage({ card, detail = false }: { card: typeof CARDS[number]; detail?: boolean }) {
  const src = cardArtSource(card, detail ? "full" : "thumbnail");
...
export function CompendiumScreen({ segments = [] }: { segments?: string[] }) {
  ...
  return <>
    <section className="compact-page-heading compendium-heading"><div><span className="eyebrow">AUTHORITATIVE REFERENCE</span><h1>Compendium</h1><p>Browse all supported cards, the official rules and glossary, and published developer rulings.</p></div><div className="compendium-source-summary"><strong>3 sets · {CARDS.length} cards</strong><span>Sources reviewed {REFERENCE_REVIEWED_AT}</span></div></section>
    <section className="compendium-toolbar"><label className="search-box large">⌕<input value={compendiumQuery} onChange={(event) => setCompendiumQuery(event.target.value)} placeholder="Search cards, rules, symbols, IDs, or rulings…" /></label><div className="tabs"><button className={compendiumTab === "cards" ? "active" : ""} onClick={() => router.push("/compendium")}>CARDS</button><button className={compendiumTab === "rules" ? "active" : ""} onClick={() => router.push("/compendium/rules")}>RULES & GLOSSARY</button><button className={compendiumTab === "rulings" ? "active" : ""} onClick={() => router.push("/compendium/rulings")}>RULINGS</button></div></section>
    {section === "cards" && <><section className="compendium-filters panel"><label>Set<select value={setCode} onChange={(event) => setSetCode(event.target.value)}><option>All</option>{Object.values(CARD_SET_INFO).map((set) => <option value={set.code} key={set.code}>{set.name}</option>)}</select></label><label>Type<select value={type} onChange={(event) => setType(event.target.value)}><option>All</option>{["Action","Flip","Hero","Evo","Character"].map((value) => <option key={value}>{value}</option>)}</select></label><label>Faction<select value={faction} onChange={(event) => setFaction(event.target.value)}><option>All</option>{["Aquos","Aurelus","Darkus","Haos","Pyrus","Ventus"].map((value) => <option key={value}>{value}</option>)}</select></label><Badge>{cards.length} RESULTS</Badge></section><section className="compendium-cards compact-card-grid">{visible.map((card) => <button className="reference-card compact" key={card.catalogId} onClick={() => router.push(`/compendium/cards/${card.slug ?? card.catalogId}`)}><span className="reference-art"><CardImage card={card} /></span><span className="reference-card-copy"><span className="reference-card-badges"><Badge tone={factionClass(card.faction)}>{card.faction}</Badge><Badge>{cardSetCode(card)}</Badge></span><strong>{card.displayName}</strong><small>{card.type} · {card.cost} Energy · {card.rarity}</small></span></button>)}</section><nav className="catalog-pagination"><button disabled={page === 1} onClick={() => setPage((value) => value - 1)}>← PREVIOUS</button><span>Page {page} of {pages}</span><button disabled={page === pages} onClick={() => setPage((value) => value + 1)}>NEXT →</button></nav></>}
