...
function CardImage({ card, detail = false }: { card: typeof CARDS[number]; detail?: boolean }) {
  const src = detail ? card.art : card.hasProvidedScan ? `/assets/cards/thumb/${card.number}.webp` : card.art;
  return <img src={src} alt={detail ? card.displayName : ""} width={detail ? 600 : 260} height={detail ? 840 : 364} loading={detail ? "eager" : "lazy"} />;
}
...
export function CompendiumScreen({ segments = [] }: { segments?: string[] }) {
  ...
  useEffect(() => { setPage(1); }, [compendiumQuery, faction, type]);
  const cards = useMemo(() => CARDS.filter((card) => {
    const text = `${card.displayName} ${card.effect} ${card.faction} ${card.type} ${card.catalogId}`.toLowerCase();
    return (!normalized || text.includes(normalized)) && (type === "All" || card.type === type) && (faction === "All" || card.factions.includes(faction as never));
  }), [faction, normalized, type]);
  ...
  if (selected) return <section className="card-detail-page"><header className="card-detail-nav"><button onClick={() => router.push("/compendium")}>← CARD RESULTS</button></header><div className="card-detail-layout"><CardImage card={selected} detail /><article className="panel"><Badge tone={factionClass(selected.faction)}>{selected.factions.join(" • ")}</Badge><h1>{selected.displayName}</h1><p className="card-effect-large"><Effect text={selected.effect} /></p><dl className="metadata-grid"><div><dt>Catalogue ID</dt><dd>{selected.catalogId}</dd></div><div><dt>Collector number</dt><dd>{selected.number}/374</dd></div><div><dt>Type</dt><dd>{selected.type}</dd></div><div><dt>Rarity</dt><dd>{selected.rarity}</dd></div><div><dt>Energy cost</dt><dd>{selected.cost}</dd></div><div><dt>Source</dt><dd>{selected.source ?? "Provided catalogue"}</dd></div></dl><div className="hero-actions"><AppButton tone="red" onClick={() => router.push(`/compendium/rulings?card=${encodeURIComponent(selected.catalogId)}`)}>OPEN RULINGS</AppButton><AppButton tone="ghost" onClick={() => void copyLink(`/compendium/cards/${selected.slug ?? selected.catalogId}`, selected.displayName)}>COPY LINK</AppButton></div></article></div></section>;
  return <>
    <PageHeader eyebrow="AUTHORITATIVE REFERENCE" title="CARD & RULES COMPENDIUM" copy="The card catalogue, official glossary, symbol reference, and published developer responses live in this route-owned bundle." art="/assets/aquos.png" />
    <section className="compendium-provenance panel"><div><span className="eyebrow">SOURCE STATUS</span><h2>REVIEWED {REFERENCE_REVIEWED_AT}</h2></div><p>Cards: supplied workbook and scans. Rules: Official Complete Rulebook and Glossary. Published answers: developer ruling document.</p><Badge tone="gold">REVISION 1</Badge></section>
    <section className="compendium-toolbar"><label className="search-box large">⌕<input value={compendiumQuery} onChange={(event) => setCompendiumQuery(event.target.value)} placeholder="Search a card, keyword, symbol, ID, or ruling…" /></label><div className="tabs"><button className={compendiumTab === "cards" ? "active" : ""} onClick={() => router.push("/compendium")}>CARDS</button><button className={compendiumTab === "rules" ? "active" : ""} onClick={() => router.push("/compendium/rules")}>RULES & GLOSSARY</button><button className={compendiumTab === "rulings" ? "active" : ""} onClick={() => router.push("/compendium/rulings")}>RULINGS</button></div></section>
    {section === "cards" && <><section className="compendium-filters panel"><label>TYPE<select value={type} onChange={(event) => setType(event.target.value)}><option>All</option>{["Action","Flip","Hero","Evo","Character"].map((value) => <option key={value}>{value}</option>)}</select></label><label>FACTION<select value={faction} onChange={(event) => setFaction(event.target.value)}><option>All</option>{["Aquos","Aurelus","Darkus","Haos","Pyrus","Ventus"].map((value) => <option key={value}>{value}</option>)}</select></label><Badge>{cards.length} RESULTS</Badge></section><section className="compendium-cards">{visible.map((card) => <article className="reference-card" key={card.catalogId}><button className="reference-art" onClick={() => router.push(`/compendium/cards/${card.slug ?? card.catalogId}`)}><CardImage card={card} /></button><div><Badge tone={factionClass(card.faction)}>{card.faction}</Badge><h2>{card.displayName}</h2><p><Effect text={card.effect} /></p><button onClick={() => router.push(`/compendium/rulings?card=${encodeURIComponent(card.catalogId)}`)}>OPEN OFFICIAL RULINGS →</button></div></article>)}</section><nav className="catalog-pagination"><button disabled={page === 1} onClick={() => setPage((value) => value - 1)}>← PREVIOUS</button><span>Page {page} of {pages}</span><button disabled={page === pages} onClick={() => setPage((value) => value + 1)}>NEXT →</button></nav></>}
