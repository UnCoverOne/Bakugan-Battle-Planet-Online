...
function DeckAreaHeader({ section, count, action }: { section: "mine" | "public"; count: number; action?: React.ReactNode }) {
  return <section className="compact-page-heading deck-area-heading"><div><span className="eyebrow">DECK MANAGEMENT</span><h1>{section === "mine" ? "My Decks" : "Public Decks"}</h1><p>{section === "mine" ? `${count} saved decks on this account or device.` : "Browse featured and player-published Battle Planet deck lists."}</p></div>{action}<nav className="deck-area-tabs" aria-label="Deck library sections"><Link className={section === "mine" ? "active" : ""} href="/decks">My Decks</Link><Link className={section === "public" ? "active" : ""} href="/decks/public">Public Decks</Link></nav></section>;
}
...
export function DeckLibraryScreen() {
  ...
  return <>
    <DeckAreaHeader section="mine" count={decks.length} action={<AppButton tone="red" onClick={create}>+ CREATE DECK</AppButton>}/>
...
function MyDeckCard({ deck, selected, onSelect, onEdit, onDuplicate, onDelete }: { deck: DeckRecord; selected: boolean; onSelect: () => void; onEdit: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  const legal = deckIsLegal(deck);
  return <article className={`deck-tile overhaul-deck-card ${selected ? "selected" : ""}`}>
    <button className="deck-card-main" onClick={onSelect}>
      <div className={`deck-cover ${factionClass(deck.factions[0] ?? "Pyrus")}`}>{lead ? <img src={cardArtSource(lead, "thumbnail")} alt={`${lead.displayName}, Lead card for ${deck.name}`}/> : <img src="/assets/cards/card-missing.svg" alt="No Lead card selected"/>}<span>{deck.visibility}</span></div>
      <div className="deck-card-copy"><h2>{deck.name}</h2><p>{deck.factions.join(" • ")}</p><div><Badge tone={legal ? "gold" : "red"}>{legal ? "LEGAL" : `${deckErrors(deck).length} ISSUES`}</Badge><small>{deck.cardIds.length} cards · {deck.bakuganIds.length} Bakugan · {deck.coreIds.length} Cores</small></div><small>Updated {formatTimestamp(deck.updatedAt)}</small></div>
    </button>
...
export function PublicDeckLibraryScreen() {
  ...
  const visible = allPublic.filter((deck) => (!query || `${deck.name} ${deck.creator} ${deck.description} ${deck.factions.join(" ")}`.toLowerCase().includes(query.toLowerCase())) && (faction === "All" || deck.factions.includes(faction)));
  ...
  return <>
    ...
    <section className="public-deck-grid">{visible.map((deck) => { const lead = deckLeadCard(deck); return <article className="panel public-deck-card" key={deck.id}><Link className={`public-deck-lead ${factionClass(deck.factions[0] ?? "Pyrus")}`} href={`/decks/public/${encodeURIComponent(deck.id)}`}>{lead ? <img src={cardArtSource(lead, "thumbnail")} alt={`${lead.displayName}, Lead card for ${deck.name}`}/> : <img src="/assets/cards/card-missing.svg" alt="No Lead card"/>}</Link><div><div className="hero-actions"><Badge tone={deckIsLegal(deck) ? "gold" : "red"}>{deckIsLegal(deck) ? "LEGAL" : "DRAFT"}</Badge><Badge>{deck.factions.join(" • ")}</Badge></div><h2>{deck.name}</h2><p className="deck-creator">by {deck.creator ?? "Community Brawler"}</p><p>{deck.description ?? "A public Battle Planet deck."}</p><small>Published {formatTimestamp(deck.publishedAt ?? deck.updatedAt)}</small><div className="public-deck-actions"><Link href={`/decks/public/${encodeURIComponent(deck.id)}`}>VIEW DECK</Link><button onClick={() => copyDeck(deck)}>COPY TO MY DECKS</button></div></div></article>; })}</section>
  </>;
}
